...
# Plot each transformation matrix
for iS in range(num_win):
    fig,ax = plt.subplots(1)
    cax = ax.imshow(W_nmf_all[iS,:,:].T,aspect='auto',\
                   vmax=120,vmin=0)
    ax.set_title('Window %d' % iS)
    ax.set_yticks(range(5))
    ax.set_xlabel('Days within sliding window',fontsize=12)
    ax.set_ylabel('Comp #',fontsize=12)
    fig.colorbar(cax)
    fig.set_figwidth(8)
    fig.set_figheight(2.5)
    plt.tight_layout()
    fig_fname = 'weights_win%02d.png' % iS
    fig.savefig(os.path.join(save_path,fig_fname),dpi=150)
    plt.close()        
...
num_win_incl = np.arange(0,10)
fig,ax = plt.subplots(1)
im=ax.imshow(R[num_win_incl[0]*num_comp:num_win_incl[-1]*num_comp,\
               num_win_incl[0]*num_comp:num_win_incl[-1]*num_comp],\
             cmap=plt.get_cmap('Spectral_r'))
for iS in range(num_comp,num_comp*(num_win_incl.shape[0]-1),num_comp):
    plt.plot((-0.5,(num_win_incl.shape[0]-1)*num_comp-0.5),(iS,iS),\
             'k--',linewidth=0.5)
for iS in range(num_comp,num_comp*(num_win_incl.shape[0]-1),num_comp):
    plt.plot((iS,iS),(-0.5,(num_win_incl.shape[0]-1)*num_comp-0.5),\
             'k--',linewidth=0.5)
ax.set_xticks(np.arange(0,(num_win_incl.shape[0]-1)*num_comp,num_comp))
ax.set_xticklabels(np.arange(num_win_incl[0],num_win_incl[-1]))
ax.set_yticks(np.arange(0,(num_win_incl.shape[0]-1)*num_comp,num_comp))
ax.set_yticklabels(np.arange(num_win_incl[0],num_win_incl[-1]))
ax.set_xlabel('Window number',fontsize=14)
ax.set_ylabel('Window number',fontsize=14)
fig.set_figwidth(6)
fig.set_figheight(6)
fig.colorbar(im)
fig_fname = 'R_win%02d-%02d.png' % (num_win_incl[0],num_win_incl[-1])
fig.savefig(os.path.join(save_path,fig_fname),dpi=150)
plt.close()
