...
figure
for icomp=1:4
    comp_svd(:,:,icomp) = reshape(U(:,icomp),37,144*3);
    subplot(4,1,icomp);
    imagesc(comp_svd(:,:,icomp));
    if icomp==1
...
% PCA
[coeff,score,latent] = pca(LL');  % row-observations (day), col-variables
% [coeff_shuffle,score_shuffle,latent_shuffle] = pca(LL_perm');
figure
for icomp=1:4
    comp_pca(:,:,icomp) = reshape(coeff(:,icomp),37,144*3);
    subplot(4,1,icomp);
    imagesc(comp_pca(:,:,icomp));
    if icomp==1
...
figure
plot(score(:,1:4),'linewidth',2)
title('PCA coefficients')


% Reshape decomp results
% 
% 
% 
% RMS_svd = nan(62,1);
...
%     RMS_svd_shuffle(irank) = norm(LL_perm - ...
%         U_shuffle(:,1:irank)*S_shuffle(1:irank,1:irank)*V_shuffle(:,1:irank)','fro');
% end
% 
% 
% 
