...
for rank=rank_all
    ...
    % Compute modified cophenetic coefficient
    H_new_norm = nan(size(H0,1),size(H0,2),length(rep_all));
    for rep = rep_all
        save_file = sprintf('%s_r%02d_betaW0.10_betaH0.10_smoothness100.00_rep%04d.mat', ...
            'rank_fixed_20190502_repeat', rank, rep);
        ...
        % Normalize H within each day
        H_new_norm(:,:,rep) = H_new./repmat(sum(H_new,1),size(H_new,1),1);
    end
    
    
    % Compute cophenetic coeff
    Y = pdist(mean(H_new_norm,3)');
    Z = linkage(Y,'average');
    [c(rank),D] = cophenet(Z,Y);
...
for icomp=1:rank
    ...
    imagesc(squeeze(H_new_norm(icomp,:,:))');
    if icomp==1
        title(sprintf('Rank = %d', rank))
    end
end
